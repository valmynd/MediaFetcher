...
class YahooIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if m_id is None: 
            ...
            webpage = self._download_webpage(info_url, video_id, 'Downloading info json')
            json_str = re.search(r'YUI.Env.JSONP.yui.*?\((.*?)\);', webpage).group(1)
            info = json.loads(json_str)
            res = info['query']['results']['mediaObj'][0]
            stream = res['streams'][0]
            video_path = stream['path']
            video_url = stream['host']
            meta = res['meta']
            video_title = meta['title']
            video_description = meta['description']
            video_thumb = meta['thumbnail']
