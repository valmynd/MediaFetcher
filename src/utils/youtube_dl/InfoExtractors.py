...
class InfoExtractor(object):
    ...
    def _request_webpage(self, url_or_request, video_id, note=None, errnote=None):
        ...
        try:
            ...
            raise ExtractorError('%s: %s' % (errnote, compat_str(err)), sys.exc_info()[2])
...
class YoutubeIE(InfoExtractor):
    ...
    def report_lang(self):
        ...
        self._downloader.to_screen('[youtube] Setting language')

    def report_login(self):
        """Report attempt to log in."""
        self._downloader.to_screen('[youtube] Logging in')

    def report_age_confirmation(self):
        """Report attempt to confirm age."""
        self._downloader.to_screen('[youtube] Confirming age')

    def report_video_webpage_download(self, video_id):
        """Report attempt to download video webpage."""
        self._downloader.to_screen('[youtube] %s: Downloading video webpage' % video_id)

    def report_video_info_webpage_download(self, video_id):
        """Report attempt to download video info webpage."""
        self._downloader.to_screen('[youtube] %s: Downloading video info webpage' % video_id)

    def report_video_subtitles_download(self, video_id):
        """Report attempt to download video info webpage."""
        self._downloader.to_screen('[youtube] %s: Checking available subtitles' % video_id)

    def report_video_subtitles_request(self, video_id, sub_lang, format):
        """Report attempt to download video info webpage."""
        self._downloader.to_screen('[youtube] %s: Downloading video subtitles for %s.%s' % (video_id, sub_lang, format))

    def report_video_subtitles_available(self, video_id, sub_lang_list):
        """Report available subtitles."""
        sub_lang = ",".join(list(sub_lang_list.keys()))
        self._downloader.to_screen('[youtube] %s: Available subtitles for video: %s' % (video_id, sub_lang))

    def report_information_extraction(self, video_id):
        """Report attempt to extract video information."""
        self._downloader.to_screen('[youtube] %s: Extracting video information' % video_id)

    def report_unavailable_format(self, video_id, format):
        """Report extracted video URL."""
        self._downloader.to_screen('[youtube] %s: Format %s not available' % (video_id, format))

    def report_rtmp_download(self):
        """Indicate the download will use the RTMP protocol."""
        self._downloader.to_screen('[youtube] RTMP download detected')
    ...
    def _real_initialize(self):
        ...
        # Log in
        login_form_strs = {
                'continue': 'http://www.youtube.com/signin?action_handle_signin=true&feature=sign_in_button&hl=en_US&nomobiletemp=1',
                'Email': username,
                'GALX': galx,
                'Passwd': password,
        ...
        try:
            ...
            self._downloader.report_error('unable to confirm age: %s' % compat_str(err))
            return

    def _extract_id(self, url):
        mobj = re.match(self._VALID_URL, url, re.VERBOSE)
        if mobj is None:
            self._downloader.report_error('invalid URL: %s' % url)
            return
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            self._downloader.report_error('unable to download video webpage: %s' % compat_str(err))
            return
        ...
        if 'token' not in video_info:
            if 'reason' in video_info:
                ...
                self._downloader.report_error('"token" parameter not in video info for unknown reason')
            return

        # Check for "rental" videos
        if 'ypc_video_rental_bar_text' in video_info and 'author' not in video_info:
            self._downloader.report_error('"rental" videos not supported')
            return
        ...
        if 'author' not in video_info:
            self._downloader.report_error('unable to extract uploader name')
            return
        ...
        if 'title' not in video_info:
            self._downloader.report_error('unable to extract video title')
            return
        ...
        if 'conn' in video_info and video_info['conn'][0].startswith('rtmp'):
            ...
            if len(existing_formats) == 0:
                self._downloader.report_error('no known formats available for video')
                return
            ...
            if req_format is None or req_format == 'best':
                ...
                if video_url_list is None:
                    self._downloader.report_error('requested format not available')
                    return
        else:
            self._downloader.report_error('no conn or url_encoded_fmt_stream_map information found in video info')
            return
...
class MetacafeIE(InfoExtractor):
    ...
    def report_disclaimer(self):
        ...
        self._downloader.to_screen('[metacafe] Retrieving disclaimer')

    def report_age_confirmation(self):
        """Report attempt to confirm age."""
        self._downloader.to_screen('[metacafe] Confirming age')

    def report_download_webpage(self, video_id):
        """Report webpage download."""
        self._downloader.to_screen('[metacafe] %s: Downloading webpage' % video_id)

    def report_extraction(self, video_id):
        """Report information extraction."""
        self._downloader.to_screen('[metacafe] %s: Extracting information' % video_id)
    ...
    def _real_initialize(self):
        ...
        try:
            ...
            self._downloader.report_error('unable to retrieve disclaimer: %s' % compat_str(err))
            return
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            self._downloader.report_error('unable retrieve video webpage: %s' % compat_str(err))
            return
        ...
        if mobj is not None:
            ...
            if mobj is None:
                self._downloader.report_error('unable to extract media URL')
                return
        ...
        if mobj is None:
            self._downloader.report_error('unable to extract title')
            return
        video_title = mobj.group(1).decode('utf-8')

        mobj = re.search(r'submitter=(.*?);', webpage)
        if mobj is None:
            self._downloader.report_error('unable to extract uploader nickname')
            return
...
class DailymotionIE(InfoExtractor):
    ...
    def report_extraction(self, video_id):
        """Report information extraction."""
        self._downloader.to_screen('[dailymotion] %s: Extracting information' % video_id)
    ...
    def _real_extract(self, url):
        ...
        for key in ['hd1080URL', 'hd720URL', 'hqURL', 'sdURL', 'ldURL', 'video_url']:
            if key in flashvars:
                max_quality = key
                self._downloader.to_screen('[dailymotion] Using %s' % key)
                break
        else:
            self._downloader.report_error('unable to extract video URL')
            return
...
class PhotobucketIE(InfoExtractor):
    ...
    def report_download_webpage(self, video_id):
        """Report webpage download."""
        self._downloader.to_screen('[photobucket] %s: Downloading webpage' % video_id)

    def report_extraction(self, video_id):
        """Report information extraction."""
        self._downloader.to_screen('[photobucket] %s: Extracting information' % video_id)
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            self._downloader.report_error('Unable to retrieve video webpage: %s' % compat_str(err))
            return
...
class YahooIE(InfoExtractor):
    ...
    def report_download_webpage(self, video_id):
        """Report webpage download."""
        self._downloader.to_screen('[video.yahoo] %s: Downloading webpage' % video_id)

    def report_extraction(self, video_id):
        """Report information extraction."""
        self._downloader.to_screen('[video.yahoo] %s: Extracting information' % video_id)
    ...
    def _real_extract(self, url, new_video=True):
        ...
        if re.match(self._VPAGE_URL, url) is None:
            ...
            if mobj is None:
                self._downloader.report_error('Unable to extract id field')
                return
            yahoo_id = mobj.group(1)

            mobj = re.search(r'\("vid", "([0-9]+)"\);', webpage)
            if mobj is None:
                self._downloader.report_error('Unable to extract vid field')
                return
        ...
        if mobj is None:
            self._downloader.report_error('unable to extract video uploader')
            return
        ...
        if mobj is None:
            self._downloader.report_error('unable to extract video thumbnail')
            return
        ...
        if mobj is None:
            self._downloader.report_error('unable to extract video description')
            return
        ...
        if mobj is None:
            self._downloader.report_error('unable to extract video height')
            return
        yv_video_height = mobj.group(1)

        mobj = re.search(r'<meta name="video_width" content="([0-9]+)" />', webpage)
        if mobj is None:
            self._downloader.report_error('unable to extract video width')
            return
...
class VimeoIE(InfoExtractor):
    ...
    def report_download_webpage(self, video_id):
        """Report webpage download."""
        self._downloader.to_screen('[vimeo] %s: Downloading webpage' % video_id)

    def report_extraction(self, video_id):
        """Report information extraction."""
        self._downloader.to_screen('[vimeo] %s: Extracting information' % video_id)
    ...
    def _real_extract(self, url, new_video=True):
        ...
        try:
            ...
            self._downloader.report_error('unable to extract info section')
            return
        ...
        for quality in ('hd', 'sd', 'other'):
            if len(files[quality]) > 0:
                ...
                self._downloader.to_screen('[vimeo] %s: Downloading %s file at %s quality' % (video_id, video_codec.upper(), video_quality))
                break
        else:
            self._downloader.report_error('no known codec found')
            return
...
class ArteTvIE(InfoExtractor):
    ...
    def report_download_webpage(self, video_id):
        """Report webpage download."""
        self._downloader.to_screen('[arte.tv] %s: Downloading webpage' % video_id)

    def report_extraction(self, video_id):
        """Report information extraction."""
        self._downloader.to_screen('[arte.tv] %s: Extracting information' % video_id)
    ...
    def extractLiveStream(self, url):
        ...
        info = self.grep_webpage(
            ...
            re.DOTALL,
            [
                (1, 'path',   'ERROR: could not extract video path: %s' % url),
                (2, 'player', 'ERROR: could not extract video player: %s' % url),
                (3, 'url',    'ERROR: could not extract video url: %s' % url)
            ]
        )
    ...
    def extractPlus7Stream(self, url):
        ...
        info = self.grep_webpage(
            ...
            0,
            [
                (1, 'url', 'ERROR: Could not find <video> tag: %s' % url)
            ]
        )
        next_url = compat_urllib_parse.unquote(info.get('url'))
        ...
        info = self.grep_webpage(
            ...
            re.DOTALL,
            [
                (1, 'id',    'ERROR: could not extract video id: %s' % url),
                (2, 'title', 'ERROR: could not extract video title: %s' % url),
                (3, 'date',  'ERROR: could not extract video date: %s' % url),
                (4, 'url',   'ERROR: could not extract video url: %s' % url)
            ]
        )
...
class GenericIE(InfoExtractor):
    ...
    def report_download_webpage(self, video_id):
        ...
        if not self._downloader.params.get('test', False):
            self._downloader.to_screen('WARNING: Falling back on generic information extractor.')
        self._downloader.to_screen('[generic] %s: Downloading webpage' % video_id)

    def report_extraction(self, video_id):
        """Report information extraction."""
        self._downloader.to_screen('[generic] %s: Extracting information' % video_id)
...
class YoutubeSearchIE(InfoExtractor):
    ...
    def _real_extract(self, query):
        ...
        if mobj is None:
            self._downloader.report_error('invalid search query "%s"' % query)
            return
        ...
        if prefix == '':
            ...
            try:
                n = int(prefix)
                if n <= 0:
                    self._downloader.report_error('invalid download number %s for query "%s"' % (n, query))
                    return
                elif n > self._max_youtube_results:
                    self._downloader.report_warning('ytsearch returns max %i results (you requested %i)' % (self._max_youtube_results, n))
    ...
    def _download_n_results(self, query, n):
        ...
        while (50 * pagenum) < limit:
            ...
            try:
                ...
                self._downloader.report_error('unable to download API page: %s' % compat_str(err))
                return
            api_response = json.loads(data)['data']

            if not 'items' in api_response:
                self._downloader.trouble('[youtube] No video results')
                return
...
class GoogleSearchIE(InfoExtractor):
    ...
    def report_download_page(self, query, pagenum):
        ...
        query = query.decode(preferredencoding())
        self._downloader.to_screen('[video.google] query "%s": Downloading page %s' % (query, pagenum))
        ...
        if prefix == '':
            ...
            try:
                ...
                if n <= 0:
                    ...
                    self._downloader.report_warning('gvsearch returns max %i results (you requested %i)' % (self._max_google_results, n))
    ...
    def _download_n_results(self, query, n):
        ...
        while True:
            ...
            try:
                ...
                self._downloader.report_error('unable to download webpage: %s' % compat_str(err))
                return
...
class YahooSearchIE(InfoExtractor):
    ...
    def report_download_page(self, query, pagenum):
        """Report attempt to download playlist page with given number."""
        query = query.decode(preferredencoding())
        self._downloader.to_screen('[video.yahoo] query "%s": Downloading page %s' % (query, pagenum))
        ...
        if prefix == '':
            ...
            try:
                ...
                if n <= 0:
                    ...
                    self._downloader.report_warning('yvsearch returns max %i results (you requested %i)' % (self._max_yahoo_results, n))
...
class YoutubePlaylistIE(InfoExtractor):
    ...
    def report_download_page(self, playlist_id, pagenum):
        """Report attempt to download playlist page with given number."""
        self._downloader.to_screen('[youtube] PL %s: Downloading page #%s' % (playlist_id, pagenum))
    ...
    def _real_extract(self, url):
        ...
        while True:
            ...
            try:
                ...
                self._downloader.report_error('Invalid JSON in API response: ' + compat_str(err))
                return

            if not 'feed' in response or not 'entry' in response['feed']:
                self._downloader.report_error('Got a malformed response from YouTube API')
                return
        ...
        if len(videos) == total:
            self._downloader.to_screen('[youtube] PL %s: Found %i videos' % (playlist_id, total))
        else:
            self._downloader.to_screen('[youtube] PL %s: Found %i videos, downloading %i' % (playlist_id, total, len(videos)))
...
class YoutubeChannelIE(InfoExtractor):
    ...
    _MORE_PAGES_INDICATOR = "Next \N{RIGHT-POINTING DOUBLE ANGLE QUOTATION MARK}"
    IE_NAME = 'youtube:channel'

    def report_download_page(self, channel_id, pagenum):
        """Report attempt to download channel page with given number."""
        self._downloader.to_screen('[youtube] Channel %s: Downloading page #%s' % (channel_id, pagenum))
...
class YoutubeUserIE(InfoExtractor):
    ...
    def report_download_page(self, username, start_index):
        ...
        self._downloader.to_screen('[youtube] user %s: Downloading video ids from %d to %d' %
...
class BlipTVUserIE(InfoExtractor):
    ...
    def report_download_page(self, username, pagenum):
        """Report attempt to download user page."""
        self._downloader.to_screen('[%s] user %s: Downloading video ids from page %d' %
    ...
    def _real_extract(self, url):
        ...
        while True:
            ...
            try:
                ...
                self._downloader.report_error('unable to download webpage: %s' % str(err))
                return
        ...
        self._downloader.to_screen("[%s] user %s: Collected %d video ids (downloading %d of them)" %
                (self.IE_NAME, username, all_ids_count, len(video_ids)))

        for video_id in video_ids:
            self._downloader.download(['http://blip.tv/'+video_id])
...
class DepositFilesIE(InfoExtractor):
    ...
    def report_download_webpage(self, file_id):
        """Report webpage download."""
        self._downloader.to_screen('[DepositFiles] %s: Downloading webpage' % file_id)

    def report_extraction(self, file_id):
        """Report information extraction."""
        self._downloader.to_screen('[DepositFiles] %s: Extracting information' % file_id)
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            self._downloader.report_error('Unable to retrieve file webpage: %s' % compat_str(err))
            return
        ...
        if (mobj is None) or (mobj.group(1) is None):
            ...
            if (mobj is not None) and (mobj.group(1) is not None):
                ...
                self._downloader.report_error('%s' % restriction_message)
            else:
                self._downloader.report_error('unable to extract download URL from: %s' % url)
            return
...
class FacebookIE(InfoExtractor):
    ...
    def report_login(self):
        """Report attempt to log in."""
        self._downloader.to_screen('[%s] Logging in' % self.IE_NAME)
    ...
    def _real_extract(self, url):
        ...
        if not m:
            raise ExtractorError('Cannot find title in webpage')
...
class BlipTVIE(InfoExtractor):
    ...
    def report_extraction(self, file_id):
        """Report information extraction."""
        self._downloader.to_screen('[%s] %s: Extracting information' % (self.IE_NAME, file_id))

    def report_direct_download(self, title):
        """Report information extraction."""
        self._downloader.to_screen('[%s] %s: Direct download detected' % (self.IE_NAME, title))
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            raise ExtractorError('ERROR: unable to download video info webpage: %s' % compat_str(err))
        if info is None: # Regular URL
            try:
                json_code_bytes = urlh.read()
                json_code = json_code_bytes.decode('utf-8')
            except (compat_urllib_error.URLError, compat_http_client.HTTPException, socket.error) as err:
                self._downloader.report_error('unable to read video info webpage: %s' % compat_str(err))
                return
            ...
            try:
                ...
                self._downloader.report_error('unable to parse video information: %s' % repr(err))
                return
...
class MyVideoIE(InfoExtractor):
    ...
    def report_extraction(self, video_id):
        """Report information extraction."""
        self._downloader.to_screen('[myvideo] %s: Extracting information' % video_id)

    def _real_extract(self,url):
        mobj = re.match(self._VALID_URL, url)
        if mobj is None:
            self._download.report_error('invalid URL: %s' % url)
            return
...
class ComedyCentralIE(InfoExtractor):
    ...
    def report_extraction(self, episode_id):
        self._downloader.to_screen('[comedycentral] %s: Extracting information' % episode_id)

    def report_config_download(self, episode_id, media_id):
        self._downloader.to_screen('[comedycentral] %s: Downloading configuration for %s' % (episode_id, media_id))

    def report_index_download(self, episode_id):
        self._downloader.to_screen('[comedycentral] %s: Downloading show index' % episode_id)
    ...
    def _real_extract(self, url):
        ...
        if dlNewest:
            ...
            mobj = re.match(self._VALID_URL, url, re.VERBOSE)
            if mobj is None:
                self._downloader.report_error('Invalid redirected URL: ' + url)
                return
            if mobj.group('episode') == '':
                self._downloader.report_error('Redirected URL is still not specific: ' + url)
                return
            epTitle = mobj.group('episode')
        ...
        if len(mMovieParams) == 0:
            ...
            if len(altMovieParams) == 0:
                self._downloader.report_error('unable to find Flash URL in webpage ' + url)
                return
            else:
        ...
        try:
            ...
            self._downloader.report_error('unable to download episode index: ' + compat_str(err))
            return
...
class EscapistIE(InfoExtractor):
    ...
    def report_extraction(self, showName):
        self._downloader.to_screen('[escapist] %s: Extracting information' % showName)

    def report_config_download(self, showName):
        self._downloader.to_screen('[escapist] %s: Downloading configuration' % showName)
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            self._downloader.report_error('unable to download webpage: ' + compat_str(err))
            return
        ...
        try:
            ...
            self._downloader.report_error('unable to download configuration: ' + compat_str(err))
            return
        ...
        try:
            ...
            self._downloader.report_error('Invalid JSON in configuration file: ' + compat_str(err))
            return
...
class CollegeHumorIE(InfoExtractor):
    ...
    IE_NAME = 'collegehumor'

    def report_manifest(self, video_id):
        """Report information extraction."""
        self._downloader.to_screen('[%s] %s: Downloading XML manifest' % (self.IE_NAME, video_id))

    def report_extraction(self, video_id):
        """Report information extraction."""
        self._downloader.to_screen('[%s] %s: Extracting information' % (self.IE_NAME, video_id))
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            self._downloader.report_error('unable to download video info XML: %s' % compat_str(err))
            return
        ...
        try:
            ...
            self._downloader.report_error('Invalid metadata XML file')
            return
        ...
        try:
            ...
            self._downloader.report_error('Invalid manifest file')
            return
...
class SoundcloudIE(InfoExtractor):
    ...
    def report_resolve(self, video_id):
        """Report information extraction."""
        self._downloader.to_screen('[%s] %s: Resolving id' % (self.IE_NAME, video_id))

    def report_extraction(self, video_id):
        """Report information extraction."""
        self._downloader.to_screen('[%s] %s: Retrieving stream' % (self.IE_NAME, video_id))
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            self._downloader.report_error('unable to download stream definitions: %s' % compat_str(err))
            return
...
class SoundcloudSetIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        mobj = re.match(self._VALID_URL, url)
        if mobj is None:
            self._downloader.trouble('ERROR: invalid URL: %s' % url)
            return
        ...
        try:
            ...
            self._downloader.trouble('ERROR: unable to download video webpage: %s' % compat_str(err))
            return
        ...
        if 'errors' in info:
            for err in info['errors']:
                self._downloader.trouble('ERROR: unable to download video webpage: %s' % compat_str(err['error_message']))
            return
        ...
        for track in info['tracks']:
            ...
            try:
                ...
                self._downloader.trouble('ERROR: unable to download stream definitions: %s' % compat_str(err))
                return
...
class MixcloudIE(InfoExtractor):
    ...
    def report_download_json(self, file_id):
        ...
        self._downloader.to_screen('[%s] Downloading json' % self.IE_NAME)
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            self._downloader.report_error('Unable to retrieve file: %s' % compat_str(err))
            return
        ...
        if req_format is None or req_format == 'best':
            for format_param in list(formats.keys()):
            ...
            if req_format not in formats:
                self._downloader.report_error('format is not available')
                return
        ...
        return [{
            ...
            'format': (format_param is None and 'NA' or format_param.decode('utf-8')),
...
class StanfordOpenClassroomIE(InfoExtractor):
    ...
    def report_download_webpage(self, objid):
        """Report information extraction."""
        self._downloader.to_screen('[%s] %s: Downloading webpage' % (self.IE_NAME, objid))
    ...
    def _real_extract(self, url):
        ...
        if mobj.group('course') and mobj.group('video'): # A specific video
            ...
            try:
                ...
                self._downloader.report_error('unable to download course info page: ' + compat_str(err))
                return
...
class MTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if mobj is None:
            self._downloader.report_error('unable to extract song name')
            return
        song_name = unescapeHTML(mobj.group(1).decode('iso-8859-1'))
        mobj = re.search(r'<meta name="mtv_an" content="([^"]+)"/>', webpage)
        if mobj is None:
            self._downloader.report_error('unable to extract performer')
            return
        ...
        if mobj is None:
            self._downloader.report_error('unable to mtvn_uri')
            return
        mtvn_uri = mobj.group(1)

        mobj = re.search(r'MTVN.Player.defaultPlaylistId = ([0-9]+);', webpage)
        if mobj is None:
            self._downloader.report_error('unable to extract content id')
            return
        ...
        try:
            ...
            self._downloader.report_error('unable to download video metadata: %s' % compat_str(err))
            return
...
class YoukuIE(InfoExtractor):
    ...
    def report_download_webpage(self, file_id):
        """Report webpage download."""
        self._downloader.to_screen('[%s] %s: Downloading webpage' % (self.IE_NAME, file_id))
...
class XNXXIE(InfoExtractor):
    ...
    def report_webpage(self, video_id):
        """Report information extraction"""
        self._downloader.to_screen('[%s] %s: Downloading webpage' % (self.IE_NAME, video_id))
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            self._downloader.report_error('unable to download video webpage: %s' % err)
            return
...
class GooglePlusIE(InfoExtractor):
    ...
    def report_extract_entry(self, url):
        ...
        self._downloader.to_screen('[plus.google] Downloading entry: %s' % url)

    def report_date(self, upload_date):
        """Report downloading extry"""
        self._downloader.to_screen('[plus.google] Entry date: %s' % upload_date)

    def report_uploader(self, uploader):
        """Report downloading extry"""
        self._downloader.to_screen('[plus.google] Uploader: %s' % uploader)

    def report_title(self, video_title):
        """Report downloading extry"""
        self._downloader.to_screen('[plus.google] Title: %s' % video_title)

    def report_extract_vid_page(self, video_page):
        """Report information extraction."""
        self._downloader.to_screen('[plus.google] Extracting video page: %s' % video_page)
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            self._downloader.report_error('Unable to retrieve entry webpage: %s' % compat_str(err))
            return
        ...
        mobj = re.search(pattern, webpage)
        if mobj is None:
            self._downloader.report_error('unable to extract video page URL')
        ...
        if len(mobj) == 0:
            self._downloader.report_error('unable to extract video links')
...
class JustinTVIE(InfoExtractor):
    ...
    def report_download_page(self, channel, offset):
        ...
        self._downloader.to_screen('[%s] %s: Downloading video information from %d to %d' %
    ...
    def _parse_page(self, url):
        try:
            ...
            self._downloader.report_error('unable to download video info JSON: %s' % compat_str(err))
            return

        response = json.loads(webpage)
        if type(response) != list:
            error_text = response.get('error', 'unknown error')
            self._downloader.report_error('Justin.tv API: %s' % error_text)
            return
        info = []
...
class FunnyOrDieIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if not m:
            self._downloader.report_error('unable to find video information')
        video_url = unescapeHTML(m.group('url'))

        m = re.search(r"<h1 class='player_page_h1'.*?>(?P<title>.*?)</h1>", webpage, flags=re.DOTALL)
        if not m:
            self._downloader.trouble('Cannot find video title')
...
class SteamIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        for vid,vtitle,thumb in zip(mweb,titles,thumbs):
            ...
            if not video_url:
                self._downloader.report_error('Cannot find video url for %s' % video_id)
            info = {
                'id':video_id,
                'url':video_url,
                'ext': 'flv',
...
class WorldStarHipHopIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if mobj is not None:
            ...
            self._downloader.trouble('ERROR: Cannot find video url for %s' % video_id)
            return
...
class RBMARadioIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if not m:
            raise ExtractorError('Cannot find metadata')
        json_data = m.group(1)
...
class YouPornIE(InfoExtractor):
    ...
    def _print_formats(self, formats):
        ...
        for format in formats:
            print(('%s\t\t%s'  % (format['ext'], format['format'])))
    ...
    def _real_extract(self, url):
        ...
        if result is None:
            self._downloader.report_warning('unable to extract video date')
            upload_date = None
        else:
        ...
        if result is None:
            raise ExtractorError('Unable to extract download list')
        ...
        self._downloader.to_screen('[youporn] Links found: %d' % len(links))
        ...
        for link in links:
            ...
            bitrate = format[1]
            format = "-".join( format )
            title = '%s-%s-%s' % (video_title, size, bitrate)
        ...
        req_format = self._downloader.params.get('format', None)
        self._downloader.to_screen('[youporn] Format: %s' % req_format)
...
class YouJizzIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if result is None:
            raise ExtractorError('ERROR: unable to extract video title')
        video_title = result.group('title').strip()
...
class EightTracksIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if not m:
            raise ExtractorError('Cannot find trax information')
        json_like = m.group(1)
...
class TEDIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if m.group('type_talk'):
            ...
            name=m.group('name')
            self._downloader.to_screen('[%s] Getting info of playlist %s: "%s"' % (self.IE_NAME,playlist_id,name))
...
class MySpassIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if url_flv_el is None:
            self._downloader.report_error('unable to extract download url')
            return
...
class LiveLeakIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if not m:
            self._downloader.report_error('unable to find video url')
            return
        video_url = m.group(1)
