...
class GenericIE(InfoExtractor):
	...
	def _real_extract(self, url):
		...
		m = re.match(r'^(?P<type>audio|video|application(?=/(?:ogg$|(?:vnd\.apple\.|x-)?mpegurl)))/(?P<format_id>[^;\s]+)',
								 content_type)
		if m:
		...
		mobj = (re.search(r'player\.ooyala\.com/[^"?]+[?#][^"]*?(?:embedCode|ec)=(?P<ec>[^"&]+)', webpage) or
						...
						re.search(
							r'OO\.Player\.create\.apply\(\s*OO\.Player\s*,\s*op\(\s*\[\s*[\'"][^\'"]*[\'"]\s*,\s*[\'"](?P<ec>.{32})[\'"]',
							webpage) or
		...
		if mobj is not None:
			...
			if embeds:
				return self.playlist_from_matches(
					...
					getter=lambda v: OoyalaIE._url_for_embed_code(smuggle_url(v['provider_video_id'], {'domain': url})),
					ie='Ooyala')
