...
class FileDownloader(object):
    ...
    def report_progress(self, percent, data_len_str, speed, eta):
        ...
        self.to_console_title('youtube-dl - %s of %s at %s ETA %s' %
    ...
    def report_finish(self, data_len_str, tot_time):
        ...
        if self.params.get('noprogress', False):
            ...
            clear_line = ('\x1b[K' if sys.stderr.isatty() and os.name != 'nt' else '')
            self.to_screen('\r%s[download] 100%% of %s in %s' %
    ...
    def _download_m3u8_with_ffmpeg(self, filename, url):
        ...
        for program in ['avconv', 'ffmpeg']:
            ...
            self.report_error('m3u8 download detected but ffmpeg or avconv could not be found')
        cmd = [program] + args
