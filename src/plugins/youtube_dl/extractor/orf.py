...
class ORFIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        flash_xml = self._search_regex('ORF.flashXML = \'(.+?)\'', webpage, 'flash xml')
        ...
        playlist_json = self._search_regex(r'playlist\': \'(\[.*?\])\'', webpage, 'playlist').replace(r'\"','"')
        ...
        for (i, (item, info)) in enumerate(zip(flash_config.findall(xpath), playlist), 1):
            ...
            for q in ['Q6A', 'Q4A', 'Q1A']:
                ...
                if video_url is not None:
                    ...
                    break
            if rtmp_url is None:
                raise ExtractorError('Couldn\'t get video url: %s' % info['id'])
            description = self._html_search_regex(
                r'id="playlist_entry_%s".*?<p>(.*?)</p>' % i, webpage,
                'description', default=webpage_description, flags=re.DOTALL)
            videos.append({
                '_type': 'video',
                'id': info['id'],
                'title': info['title'],
