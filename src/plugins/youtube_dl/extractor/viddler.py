...
class ViddlerIE(InfoExtractor):
    ...
    _TEST = {
        ...
        'file': '43903784.mp4',
        'md5': 'fbbaedf7813e514eb7ca30410f439ac9',
        'info_dict': {
    ...
    def _real_extract(self, url):
        ...
        embed_url = mobj.group('domain') + '/embed/' + video_id
        webpage = self._download_webpage(embed_url, video_id)

        video_sources_code = self._search_regex(
            r"(?ms)sources\s*:\s*(\{.*?\})", webpage, 'video URLs')
        video_sources = json.loads(video_sources_code.replace("'", '"'))

        formats = [{
            'url': video_url,
            'format': format_id,
        } for video_url, format_id in list(video_sources.items())]

        title = self._html_search_regex(
            r"title\s*:\s*'([^']*)'", webpage, 'title')
        uploader = self._html_search_regex(
            r"authorName\s*:\s*'([^']*)'", webpage, 'uploader', fatal=False)
        duration_s = self._html_search_regex(
            r"duration\s*:\s*([0-9.]*)", webpage, 'duration', fatal=False)
