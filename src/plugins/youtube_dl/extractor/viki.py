...
class VikiIE(VikiBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if titles:
            title = titles.get('en') or titles[list(titles.keys())[0]]
        if not title:
            title = 'Episode %d' % video.get('number') if video.get('type') == 'episode' else video.get('id') or video_id
            container_titles = video.get('container', {}).get('titles')
            if container_titles:
                container_title = container_titles.get('en') or container_titles[list(container_titles.keys())[0]]
                title = '%s - %s' % (container_title, title)

        descriptions = video.get('descriptions')
        description = descriptions.get('en') or descriptions[list(titles.keys())[0]] if descriptions else None
        ...
        thumbnails = []
        for thumbnail_id, thumbnail in list(video.get('images', {}).items()):
            thumbnails.append({
                'id': thumbnail_id,
        ...
        subtitles = {}
        for subtitle_lang, _ in list(video.get('subtitle_completions', {}).items()):
        ...
        formats = []
        for format_id, stream_dict in list(streams.items()):
            height = self._search_regex(
                r'^(\d+)[pP]$', format_id, 'height', default=None)
            for protocol, format_dict in list(stream_dict.items()):
                if format_id == 'm3u8':
...
class VikiChannelIE(VikiBaseIE):
    ...
    def _real_extract(self, url):
        ...
        description = descriptions.get('en') or descriptions[list(descriptions.keys())[0]]
