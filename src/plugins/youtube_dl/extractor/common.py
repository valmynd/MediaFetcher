...
class InfoExtractor(object):
    ...
    def _check_formats(self, formats, video_id):
        if formats:
            formats[:] = [f for f in formats if self._is_valid_url(
                    f['url'], video_id,
    ...
    def _extract_f4m_formats(self, manifest_url, video_id, preference=None, f4m_id=None,
        ...
        for i, media_el in enumerate(media_nodes):
            ...
            formats.append({
                'format_id': '-'.join([_f for _f in [f4m_id, compat_str(i if tbr is None else tbr)] if _f]),
                'url': manifest_url,
                'ext': 'flv',
                'tbr': tbr,
    ...
    def _extract_m3u8_formats(self, m3u8_url, video_id, ext=None,
        ...
        formats = [{
            'format_id': '-'.join([_f for _f in [m3u8_id, 'meta'] if _f]),
            'url': m3u8_url,
            'ext': ext,
            'protocol': 'm3u8',
