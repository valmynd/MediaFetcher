...
class BrightcoveIE(InfoExtractor):
    ...
    def _build_brighcove_url(cls, object_str):
        ...
        if fv_el is not None:
            flashvars = dict(
                (k, v[0])
                for k, v in list(compat_parse_qs(fv_el.attrib['value']).items()))
        else:
            flashvars = {}
    ...
    def _extract_brightcove_urls(cls, webpage):
        ...
        matches = re.findall(
            ...
            webpage)
        if matches:
            return list([_f for _f in [cls._build_brighcove_url(m) for m in matches] if _f])

        return list([_f for _f in [
            cls._build_brighcove_url_from_js(custom_bc)
            for custom_bc in re.findall(r'(customBC\.createVideo\(.+?\);)', webpage)] if _f])
