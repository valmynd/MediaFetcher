...
def parseOpts(overrideArguments=None):
    ...
    general.add_option(
        '--cache-dir', dest='cachedir', default='~/.youtube-dl/cache',
...
def _real_main(argv=None):
    ...
    if opts.cookiefile is None:
        ...
        try:
            ...
            if opts.verbose:
                traceback.print_exc()
            write_string('ERROR: unable to open cookie file\n')
            sys.exit(101)
    # Set user agent
    ...
    if opts.batchfile is not None:
        try:
            ...
            if opts.verbose:
                write_string('[debug] Batch file urls: ' + repr(batchurls) + '\n')
        except IOError:
    ...
    try:
        ...
        parser.error('invalid playlist start number specified')
    try:
        opts.playlistend = int(opts.playlistend)
        if opts.playlistend != -1 and (opts.playlistend <= 0 or opts.playlistend < opts.playliststart):
            raise ValueError('Playlist end must be greater than playlist start')
    except (TypeError, ValueError) as err:
        parser.error('invalid playlist end number specified')
    ...
    if opts.verbose:
        ...
        write_string('[debug] Proxy map: ' + str(proxy_handler.proxies) + '\n')
    ...
    if opts.cookiefile is not None:
        try:
            ...
            sys.exit('ERROR: unable to save cookie jar')
