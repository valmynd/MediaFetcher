...
class YoutubeDL(object):
    ...
    def process_ie_result(self, ie_result, download=True, extra_info={}):
        ...
        if result_type == 'video':
            ...
            force_properties = dict(
                (k, v) for k, v in list(ie_result.items()) if v is not None)
            for f in ('_type', 'url'):
    ...
    def _apply_format_filter(self, format_spec, available_formats):
        ...
        operator_rex = re.compile(r'''(?x)\s*\[
            ...
            \]$
            ''' % '|'.join(map(re.escape, list(OPERATORS.keys()))))
        ...
        if not m:
            ...
            str_operator_rex = re.compile(r'''(?x)\s*\[
                ...
                \s*\]$
                ''' % '|'.join(map(re.escape, list(STR_OPERATORS.keys()))))
    ...
    def process_video_result(self, info_dict, download=True):
        ...
        for format_id, ambiguous_formats in list(formats_dict.items()):
    ...
    def process_subtitles(self, video_id, normal_subtitles, automatic_captions):
        ...
        if automatic_captions and self.params.get('writeautomaticsub'):
            for lang, cap_info in list(automatic_captions.items()):
        ...
        if self.params.get('allsubtitles', False):
            requested_langs = list(available_subs.keys())
        else:
        ...
        for lang in requested_langs:
            ...
            for ext in formats_preference:
                if ext == 'best':
                    f = formats[-1]
                    break
                matches = list([f for f in formats if f['ext'] == ext])
                if matches:
                    f = matches[-1]
                    break
            else:
    ...
    def process_info(self, info_dict):
        ...
        if subtitles_are_requested and info_dict.get('requested_subtitles'):
            ...
            for sub_lang, sub_info in list(subtitles.items()):
    ...
    def filter_requested_info(info_dict):
        return dict(
            (k, v) for k, v in list(info_dict.items())
    ...
    def list_subtitles(self, video_id, subtitles, name='subtitles'):
        ...
        self.to_screen(render_table(
            ...
            [[lang, ', '.join(f['ext'] for f in reversed(formats))]
                for lang, formats in list(subtitles.items())]))
